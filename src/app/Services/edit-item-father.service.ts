...
export class EditItemFatherService {

  private updateChildURL = 'https://polar-stream-42470.herokuapp.com/item/';
  ...
  checkValuesIntegrity(item: CreateItemChild): boolean {
    if (item.productId && item._id && item.category && item.material && item.finishing && item.height && item.width && item.length) {
      return true;
    } else {
      return false;
    }
  ...
  editItem(item: CreateItemChild): Observable<any> {
    return this.http.put<any>(this.updateChildURL + item._id, item, httpOptions).pipe(catchError((error) => {
